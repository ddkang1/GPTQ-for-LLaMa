...
@torch.no_grad()
def starcoder_sequential(model, dataloader, dev, level):
...
@torch.no_grad()
def starcoder_eval(model, testenc, dev, dataset_name):
    def get_batch_iterator(data, nsamples):
...
def starcoder_pack(model, quantizers, wbits, groupsize):
...
if __name__ == "__main__":
    ...
    if args.load:
        ...
        model = get_starcoder(args.model, args.wbits)
        model.eval()
    ...
    if not args.load and args.wbits < 16 and not args.nearest:
        tick = time.time()
        quantizers = starcoder_sequential(model, dataloader, DEV, args.optimization_level)
        print(time.time() - tick)
        starcoder_pack(model, quantizers, args.wbits, args.groupsize)
    ...
    if args.eval:
        ...
        for dataset in datasets:
            ...
            print(dataset)
            starcoder_eval(model, testloader, DEV, dataset)
